...
public final class DeviceManagementHttpEndpoint extends AbstractHttpEndpoint<ServiceConfigProperties> {
    ...
    public String getName() {
        return DeviceManagementConstants.DEVICES_HTTP_ENDPOINT;
    }
    ...
    private void doGetDevice(final RoutingContext ctx) {
        ...
        final JsonObject requestMsg = EventBusMessage.forOperation(DeviceManagementConstants.ACTION_GET)
                .setTenant(tenantId)
    ...
    private void doCreateDevice(final RoutingContext ctx) {
        ...
        if (deviceId != null) {
            if (!(deviceId instanceof String)) {
                HttpUtils.badRequest(ctx, String.format("'%s' must be a string",
                        DeviceManagementConstants.FIELD_PAYLOAD_DEVICE_ID));
            }
        }
        ...
        logger.debug("creating device [tenant: {}, device: {}, payload: {}]", tenantId, deviceId,
                payload);
        final JsonObject requestMsg = EventBusMessage.forOperation(DeviceManagementConstants.ACTION_CREATE)
                .setTenant(tenantId)
                .setDeviceId(deviceId)
    ...
    private void doUpdateDevice(final RoutingContext ctx) {
        ...
        final JsonObject payload = ctx.get(KEY_REQUEST_BODY);
        if (payload != null) {
            payload.remove(DeviceManagementConstants.FIELD_PAYLOAD_DEVICE_ID);
        }
        final String tenantId = getTenantParam(ctx);
        logger.debug("updating device [tenant: {}, device: {}, payload: {}]", tenantId, deviceId,
                payload);
        final JsonObject requestMsg = EventBusMessage.forOperation(DeviceManagementConstants.ACTION_UPDATE)
                .setTenant(tenantId)
                .setDeviceId(deviceId)
                .setJsonPayload(payload)
    ...
    private void doDeleteDevice(final RoutingContext ctx) {
        ...
        final JsonObject requestMsg = EventBusMessage.forOperation(DeviceManagementConstants.ACTION_DELETE)
                .setTenant(tenantId)
                .setDeviceId(deviceId)
                .setResourceVersion(ctx.get(KEY_RESOURCE_VERSION))
                .toJson();
